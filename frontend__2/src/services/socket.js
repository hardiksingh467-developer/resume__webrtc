...
const socket = io("https://webrtc-backend-1-8971.onrender.com", {
  // const socket = io("http://192.168.136.1:3015", {
  autoConnect: false,
...
export const joinRoom = (appointment_id, name) => {
  console.log(
    "joining room with appointment_id: ",
    appointment_id,
    " and name: ",
    name
  );
...
export const saveChats = async (appointment_id, chats) => {
  try {
    const response = await axios.put(
      `https://webrtc-p2p-mesh.onrender.com/api/v1/signalling/save-chats/${appointment_id}`,
      { chats }
    );
