...
import { connectSocket, joinRoom, listenForJoinSuccess, sendOffer, listenForAnswer, listenForErrors, listenForOffer, sendAnswer, sendICE, listenForICE, disconnectSocket, listenForUserLeft, resetSockets, saveChats } from "../services/socket";
...
const ThirdRoom3 = ({ appointmentId, name }) => {
	...
	const answerPeer = (incoming) => {
		...
		console.log(incoming?.name || "Ghost");
	...
	const handleReceiveMessage = (e) => {
		...
		messagesRef.current.push({ isMe: false, message: e.data, old_text: true, user_id: otherUser.current });
	...
	const sendMessage = (e) => {
		...
		setMessages(messages => [...messages, { isMe: true, user_id: userId.current, message: text }]);
		messagesRef.current.push({ isMe: true, user_id: userId.current, message: text });
		setText("");
	}
							...
							focus === "chat" && (
								<form className="flex flex-col w-full text-white h-full" onSubmit={sendMessage}>
									<div className="grow overflow-x-hidden w-full">
										<div className="p-2 h-dvh max-h-[calc(100vh_-_11.5rem)] overflow-y-auto overflow-x-hidden no__scrollbar text-wrap object-contain">
											{messages?.map((item, index) => {
												...
												return (
												<div className={`w-fit max-w-[80%] text-wrap break-all my-1.5 p-3 text-black h-fit ${(!!(item.user_id) && item?.user_id === userId.current) ? "bg-[#E6E8ED] ml-auto rounded-l-2xl rounded-tr-2xl" : "bg-[#FFD5B3] mr-auto rounded-r-2xl rounded-tl-2xl"}`} key={`${index}_${item?.message}`}>
									...
									<div className="flex w-full h-fit gap-3 justify-between items-center">
										...
										<button type="submit" className="min-w-fit cursor-pointer rounded-full hover:bg-orange-200 p-2 aspect-square bg-orange-600 hover:text-blue-600 border border-solid border-orange-600">
